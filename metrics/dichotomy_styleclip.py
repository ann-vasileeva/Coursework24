...
def apply_styleclip_batches(net, model, latents, strength, fs3, latent_idx, create_photos=True, output_folder="styleclip_generated"):
    ...
    target='smiling face' 
    #target = 'face with gray hair'
    #target = 'face with wavy hair'
    #target = 'face with earrings'
    #target = 'male face'
    #target = 'face with big nose'
    #target = 'face with glasses'
    classnames=[target,neutral]
    dt=GetDt(classnames,model) #clip
...
def binary_search_for_s(net, model, latents_to_edit, fs3, strengths, inds_without, method_name="ganspace", direction_name="smile", celeba_attr="Smiling"): #unimodal function
    # values = []
    # f_val = []
    known_fid_scores = {}
    left = 0
    ...
    #count mid value
    apply_styleclip_batches(net.decoder, model, latents_to_edit, strengths[mid], fs3, inds_without, create_photos=True, output_folder=f"styleclip_generated_{direction_name}")
...
#    print(result.stderr)
    ...
    while left < right:
      ...
      if strengths[y] not in known_fid_scores:
        apply_styleclip_batches(net.decoder, model, latents_to_edit, strengths[y], fs3, inds_without, create_photos=True, output_folder=f"styleclip_generated_{direction_name}")
      ...
      if fid_score_y <= fid_score_mid:
        ...
        if strengths[z] not in known_fid_scores:
          apply_styleclip_batches(net.decoder, model, latents_to_edit, strengths[z], fs3, inds_without, create_photos=True, output_folder=f"styleclip_generated_{direction_name}")
...
strengths = np.linspace(0,5,21)

binary_search_for_s(net, model, latents_to_edit, fs3, strengths, inds_without, method_name="stylespace", direction_name="smile",celeba_attr="Smiling")
#binary_search_for_s(net, model, latents_to_edit, fs3, strengths, inds_without, method_name="stylespace", direction_name="gray_hair",celeba_attr="Gray_Hair")
#binary_search_for_s(net, model, latents_to_edit, fs3, strengths, inds_without, method_name="stylespace", direction_name="earrings",celeba_attr="Wearing_Earrings")
