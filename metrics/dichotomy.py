...
def apply_ganspace_batches(net, latents, ganspace_pca, direction, latent_idx, create_photos=True,
                           output_folder="ganspace_generated_smile"):
    b_sz = 25
    n_batches = len(latents) // b_sz
    if len(latents) % b_sz != 0:
        n_batches += 1
        
    strength = np.abs(direction[3])

    if not os.path.exists(f"/home/ayavasileva/data/{output_folder}_strength{strength}"):
        os.mkdir(f"/home/ayavasileva/data/{output_folder}_strength{strength}")

    for batch in range(n_batches):
        edit_latents = ganspace.edit(latents[batch * b_sz: (batch + 1) * b_sz].to("cuda"), ganspace_pca, [direction])
        if create_photos:
            with torch.no_grad():
                ...
                for n_image, image in enumerate(images):
                    ...
                    new_image.save(
                        f"/home/ayavasileva/data/{output_folder}_strength{strength}/{latent_idx[n_image + batch * b_sz]}.jpg")
...
def binary_search_for_s(net, latents_to_edit, ganspace_pca, strengths, inds_without, direction, method_name="ganspace", direction_name="smile", celeba_attr="Smiling"): #unimodal function
    ...
    #count mid value
    cur_direction_mid = (direction[0], direction[1], direction[2],
                          strengths[mid])

    apply_ganspace_batches(net.decoder, latents_to_edit, ganspace_pca,
                            [cur_direction_mid], inds_without, create_photos=True,
                            output_folder=f"ganspace_generated_{direction_name}")

    os.chdir("/home/ayavasileva/sfe-main/launch")

    cur_edit_path = f"/home/ayavasileva/data/ganspace_generated_{direction_name}_strength{np.abs(strengths[mid])}"
    ...
    while left < right:
      ...
      if strengths[y] not in known_fid_scores:
        cur_direction_right = (direction[0], direction[1], direction[2],
                            strengths[y])

        apply_ganspace_batches(net.decoder, latents_to_edit, ganspace_pca,
                              [cur_direction_right], inds_without, create_photos=True,
                              output_folder=f"{method_name}_generated_{direction_name}")

        cur_edit_path = f"/home/ayavasileva/data/ganspace_generated_{direction_name}_strength{np.abs(strengths[y])}"
      ...
      if fid_score_y <= fid_score_mid:
        ...
        if strengths[z] not in known_fid_scores:
          cur_direction_z = (direction[0], direction[1], direction[2],
                              strengths[z])

          apply_ganspace_batches(net.decoder, latents_to_edit, ganspace_pca,
                                [cur_direction_z], inds_without, create_photos=True,
                                output_folder=f"ganspace_generated_{direction_name}")

          cur_edit_path = f"/home/ayavasileva/data/ganspace_generated_{direction_name}_strength{np.abs(strengths[z])}"
...
directions = {'smile': (46, 4, 5, -25), "gray_hair": (57, 7, 10, 0), "curly_hair": (47, 3, 6, 20),
              "nose_length": (51, 4, 5, -20)}

from editings import ganspace

ganspace_pca = torch.load('/home/ayavasileva/encoder4editing/editings/ganspace_pca/ffhq_pca.pt')
...
strengths = range(-1, -35, -2)

binary_search_for_s(net, latents_to_edit, ganspace_pca, strengths, inds_without, direction, method_name="ganspace", direction_name="smile")
